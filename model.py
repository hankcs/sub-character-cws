...
parser.add_argument("--old-dataset", dest="old_dataset", help="Path to old dataset for incremental training, used to "
...
if options.old_dataset is not None:
    old_dataset = pickle.load(open(options.old_dataset, 'rb'))
    ow2i = dataset["w2i"]
    ot2i = dataset["t2i"]
    oc2i = dataset["c2i"]

    def merge_map(x2i, y2i):
        for x in x2i:
            if x not in y2i:
                y2i[x] = len(y2i)
    ...
    oi2w = utils.to_id_list(w2i)  # Inverse mapping
    oi2t = utils.to_id_list(t2i)
    oi2c = utils.to_id_list(c2i)

    def remap_instances(instances):
        for instance in instances:
            osent = [ow2i[i2w[i]] for i in instance.sentence]
            del instance.sentence[:]
            instance.sentence.extend(osent)

    remap_instances(training_instances)
    remap_instances(dev_instances)
    remap_instances(test_instances)
    w2i = ow2i
    t2i = ot2i
    c2i = oc2i
    i2w = oi2w
